...
BOOL ConvertToShellcode(DWORD userFunction, DWORD userLength, DWORD flags, LPSTR &outBytes, DWORD &outLength, BOOL bIs64)
{
...
//#endif
	...
	if (bIs64)
	{
		...
		if (rdiShellcode == NULL || rdiShellcodeLength == 0) return false;
		...
		dllOffset = sizeof(bootstrap) - i + rdiShellcodeLength + userLength;
		...
		bootstrap[i++] = 0x81;
		bootstrap[i++] = 0xc0;
		userDataLocation = dllOffset - userLength;
		...
		// User data	(not set)
		// DLL bytes	(not set)
		outLength = rdiShellcodeLength + sizeof(bootstrap);
...
int main(int argc, char *argv[], char *envp[])
{
	...
	//if (ConvertToShellcode(HashFunctionName("Init"), 0x4000, SRDI_CLEARHEADER | SRDI_CLEARMEMORY | SRDI_OBFUSCATEIMPORTS, finalShellcode, finalSize, FALSE))
	//{
	//	printf("x86 shellcode(%llu):\n", (unsigned long long )finalSize);
	//	for (size_t i = 0; i < finalSize; i++)
	//	{
	//		printf("\\x%02X", (unsigned char)finalShellcode[i]);
	//	}
	//	printf("\n");
	//}

	if (ConvertToShellcode(HashFunctionName("Init"), 0x4000, SRDI_CLEARHEADER | SRDI_CLEARMEMORY | SRDI_OBFUSCATEIMPORTS, finalShellcode, finalSize, TRUE))
	{
		printf("x64 shellcode(%llu):\n", (unsigned long long)finalSize);
		for (size_t i = 0; i < finalSize; i++)
		{
			printf("\\x%02X", (unsigned char)finalShellcode[i]);
		}
